...
let userId =
  localStorage.getItem("userID") || Math.floor(Math.random() * 9999999) + 1;
...
window.onload = function () {
  ...
  if (localStorage.getItem("dailyWordLastTime")) {
    ...
    if (currentDay !== lastPlayedDay) {
      ...
      collectData(
        "Stored daily word cleared automatically",
        "dictum_auto_clear"
      );
    }
    ...
    if (currentDay - lastPlayedDay > 1) {
      ...
      collectData(
        "Lost streak with lastPlayedDay of " +
          lastPlayedDay +
          " and currentDay " +
          currentDay,
...
function buildStats() {
  ...
  document.getElementById("streaks-answers-correct").innerHTML =
...
function addLetter(letter) {
  ...
  for (let i = 0; i < gameBoard.length; i++) {
    if (
      4 < i &&
      !document.getElementById("row1").classList.contains("is-completed")
    ) {
      break;
    }
    ...
    if (
      9 < i &&
      !document.getElementById("row2").classList.contains("is-completed")
    ) {
      break;
    }
    ...
    if (
      14 < i &&
      !document.getElementById("row3").classList.contains("is-completed")
    ) {
      break;
    }
    ...
    if (
      19 < i &&
      !document.getElementById("row4").classList.contains("is-completed")
    ) {
      break;
    }
    ...
    if (
      24 < i &&
      !document.getElementById("row5").classList.contains("is-completed")
    ) {
      break;
    }
    ...
    if (
      29 < i &&
      !document.getElementById("row6").classList.contains("is-completed")
    ) {
      break;
    }
...
function checkAnswer() {
  ...
  let gameBoardId = document.querySelectorAll(".game-row.is-not-completed")[0]
    .id;
  let fullSubmission = [];
  ...
  for (let i = 0; i < 5; ++i) {
    ...
    else if (targetWord[i] in letterCounts) {
  ...
  for (let i = 0; i < 5; ++i) {
    ...
    if (correctionClasses[i] != "is-correct-place") {
      ...
      if (letterCounts.get(fullSubmission[i]) > 0) {
        ...
        letterCounts.set(
          fullSubmission[i],
          letterCounts.get(fullSubmission[i]) - 1
        );
      } else {
  ...
  for (let i = 0; i < 5; i++) {
    setTimeout(function timer() {
      ...
      document
        ...
        .classList.add(correctionClasses[i]);
      ...
      if (i === 4) {
        ...
        if (fullSubmission.join("") === targetWord) {
          ...
          document.querySelectorAll(".game-row.is-completed").length === 6
        ) {
          document.getElementById("copy-stats").style.display = "flex";
          canType = false;
          finishAnswer("wrong");
          ...
          collectData(copyDailyStat(true));
          document.getElementById("game-notice-text").innerHTML =
...
function streakCalculation(isCorrect) {
  ...
  if (isCorrect) {
    ...
    collectData(
      "Streak continued at " + currentStreak,
      "dictum_streak_continued"
    );
...
function copyDailyStat(string) {
  let board = [];
  document
    .querySelectorAll(".game-row.is-completed .game-tile")
    .forEach((tile) => {
  ...
  let digit = isDailyWord
    ? parseInt(timeDifference(Date.now(), unix)) + " "
    : "";
...
function correctAnswer() {
  ...
  setTimeout(function () {
    document
      .querySelectorAll(".game-row.is-answer")[0]
...
function resetGame() {
  ...
  document.getElementById("copy-stats-link").innerHTML =
...
function finishAnswer(answer) {
  ...
  document.getElementById("word-inflection").innerHTML = findInflection(
    selectedWord.type
  );
  ...
  if (isDailyWord) {
    ...
    let answeredCorrectly =
      parseInt(localStorage.getItem("answeredCorrectly")) || 0;
  ...
  let streaksGamesPlayed =
    parseInt(localStorage.getItem("streaksGamesPlayed")) + 1 || 1;
  let streaksAnsweredCorrectly =
    parseInt(localStorage.getItem("streaksAnsweredCorrectly")) || 0;
  let streaksScoreArray = JSON.parse(
    localStorage.getItem("streaksScoreArray")
  ) || {
    1: 0,
    2: 0,
    3: 0,
  ...
  let streaksHighScore =
    parseInt(localStorage.getItem("streaksHighScore")) || 0;
  ...
  if (
    parseInt(document.getElementById("current-streak").textContent) >
    streaksHighScore
  ) {
    streaksHighScore = parseInt(
      document.getElementById("current-streak").textContent
    );
  }
